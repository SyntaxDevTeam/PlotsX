...
class UUIDManager(private val plugin: PlotsX) {
    ...
    private fun fetchPlayerNameFromPlayerDB(uuid: String): String? {
        ...
        return try {
            ...
            if (connection.responseCode == 200) {
                ...
                reader.close()
                val jsonObject = gson.fromJson(response, JsonObject::class.java)
                val playerName = jsonObject.getAsJsonObject("data").getAsJsonObject("player").get("username").asString
                plugin.logger.debug("Player name from PlayerDB API: $playerName")
                playerName
            } else {
                plugin.logger.err("Failed to fetch player name from PlayerDB API. Response code: ${connection.responseCode}")
                null
            }
        } catch (e: Exception) {
            plugin.logger.err("Error: $e")
